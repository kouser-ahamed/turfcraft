...
const ManageEditFacilitiesPage = ({ facility }) => {
  ...
  const [timeSlots, setTimeSlots] = useState(() => facility?.availableTimeSlots ?? []);
  ...
  useEffect(() => {
    const loadFacility = async () => {
      ...
      try {
        ...
        setTimeSlots(Array.isArray(data?.availableTimeSlots) ? data.availableTimeSlots : []);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/facility/${facilityData?._id || id}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    isLoading ? (
      <div className="mx-auto my-8 max-w-4xl rounded-3xl border border-slate-100 bg-white p-8 text-center shadow-xl">
        <p className="text-sm font-semibold text-slate-500">Loading facility details...</p>
      </div>
    ) : (
    <div className="max-w-4xl mx-auto my-8 p-4 md:p-8 bg-white rounded-3xl border border-slate-100 shadow-xl">
      ...
      <div className="flex items-center gap-3 pb-6 mb-8 border-b border-slate-100">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Update the details and available timings of your facility.</p>
        </div>
      </div>
